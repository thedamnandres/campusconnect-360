...
def enqueue_event(db, event_type: str, payload: dict, deduplication_key: str, correlation_id: str | None = None):
    existing = db.query(OutboxEvent).filter(OutboxEvent.deduplication_key == deduplication_key).first()
    if existing:
        return existing
        ...
        async with connection:
            ...
            for outbox_event in events:
                try:
                    ...
                    await exchange.publish(message, routing_key=f"campusconnect.{outbox_event.event_type.lower()}")
                    ...
                    db.commit()
                    logger.info("[OUTBOX PUBLISHED] %s | id=%s", outbox_event.event_type, outbox_event.id)
                    ...
                    if failed:
                        ...
                        failed.next_attempt_at = datetime.utcnow() + timedelta(seconds=min(30, 2 ** min(failed.attempts, 5)))
                        db.commit()
